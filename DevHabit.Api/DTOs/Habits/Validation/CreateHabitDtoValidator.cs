...
namespace DevHabit.Api.DTOs.Habits.Validation;
...
public class CreateHabitDtoValidator : AbstractValidator<CreateHabitDto>
{
    ...
    public CreateHabitDtoValidator()
    {
        ...
        RuleFor(x => x.Type).IsInEnum().WithMessage("Invalid habit type.");
        
        RuleFor(x => x.Frequency.Type).IsInEnum().WithMessage("Invalid frequency period.");

        RuleFor(x => x.Frequency.TimesPerPeriod).GreaterThan(0).WithMessage("Frequency must be greater than 0.");
        
        RuleFor(x => x.Target.Value).GreaterThan(0).WithMessage("Target value must be greater than 0.");

        RuleFor(x => x.Target.Unit).NotEmpty().Must(unit => AllowedUnits.Contains(unit.ToLowerInvariant()))
            .WithMessage($"Unit must be one of: {string.Join(", ", AllowedUnits)}.");

        RuleFor(x => x.EndDate).Must(date => date is null || date.Value > DateOnly.FromDateTime(DateTime.UtcNow))
            .WithMessage("End date must be in the future");

        When(x => x.Milestone is not null, () =>
        {
            RuleFor(x => x.Milestone!.Target).GreaterThan(0).WithMessage("Milestone target must be greater than 0.");
        });

        RuleFor(x => x.Target.Unit).Must((dto, unit) => IsTargetUnitCompatibleWithType(dto.Type, unit))
